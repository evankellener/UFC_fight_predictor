...
- **Apr 23 2026 (walk-forward 4-fold validation)**: `scripts/walk_forward_4fold.py` runs 4 folds with 7-year training windows + 6-month test slices (2024-04 → 2026-04). Leakage audit at runtime: hard `assert` that `train_max < test_start` and zero bout overlap between train/test. §4 (scaler, imputer, LR, calibrator) all refit per-fold on training data only; test is never touched during tuning. **Known methodology gap**: the Poisson-Gamma / Beta-Binomial τ hyperparameters in `mma_ai_config.py` are held fixed across folds rather than re-optimized per-fold. Full per-fold τ re-optimization via `src/optimize_taus.py` would cost ~2-3 hours of Optuna compute. τ has ~30 scalar hyperparameters vs LR's 207 learned coefficients, so the effective capacity (and thus leakage risk) is second-order relative to the LR refit. Results (522 out-of-sample fights aggregated): 71.2% ± 4.3pp accuracy, with clear temporal drift (fold 1 = 76.4%, fold 4 = 65.1%).

- **Apr 27 2026 (per-fold τ re-optimization, 8-fold × 3-mo)**: `scripts/reoptimize_tau_8fold.py` extends the previous 4-fold τ-fixed walk-forward by re-optimizing the Poisson-Gamma + Beta-Binomial τ subset (11 scalars: 6 PG + 5 BB) per fold via Optuna (30 trials/fold, ~5h total). Inner-validation = last 6 months of each fold's 4-yr training window, split into 2× 3-month chunks. **Strict leakage guards**: (a) Optuna only sees inner-validation slices that are INSIDE the fold's training window — the fold's actual test window is NEVER read during τ search [§1]; (b) feature CSV (`data/tmp/mmaai_features.csv`) is backed up before run and atomically restored at the end so a τ-mutated feature matrix from one fold can't bleed into another fold's evaluation; (c) imputer/scaler/calibrator refit per inner fold AND per outer fold on training data only [§4]. **τ search subspace**: curated to the 11 scalars known from prior work to dominate signal (sig_str_land, head_land, body_land, td_land, sub_att, kd PG; ko, win, decision, sub_land, ctrl BB) — full 30-scalar search would be 6 × cost. Larger search risks overfitting the inner-validation small sample (~150-200 fights per fold's inner-train slice). Results recorded at `results/tau_per_fold_8.json`. Compare-against-baseline: `results/parlay_lambda120_8fold_4yr_results.json` (τ FIXED, men-only PARLAY-2 edge≥5pp pooled +27.25% on n=55 / edge≥10pp +39.47% on n=37).
