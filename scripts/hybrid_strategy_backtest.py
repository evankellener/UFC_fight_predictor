"""Backtest hybrid strategies that mix parlay (when confident) and singles
(when not). Compare to pure-parlay and pure-single baselines.

Hybrid variants tested:
  H1 (user's spec): top-2 parlay if BOTH legs ≥10pp; else single if best ≥10pp; else skip
  H2 (relaxed):     top-2 parlay if BOTH legs ≥10pp; else single if best ≥5pp;  else skip
  H3 (compromise):  top-2 parlay if BOTH legs ≥7pp;  else single if best ≥10pp; else skip
  H4 (aggressive):  top-2 parlay if BOTH legs ≥5pp;  else single if best ≥10pp; else skip

Baselines:
  STRAIGHT edge≥5pp (every qualifying single)
  STRAIGHT edge≥10pp (only high-confidence singles)
  PARLAY-2 edge≥5pp top-2 (current ship)
  PARLAY-2 edge≥10pp top-2 (high-confidence parlays only)

Per strategy: pooled ROI, n_bets, hit_rate, fold breakdown, F8 ROI.
Uses λ=1.50, 4-yr training, men-only, 8 folds × 3-mo.
"""
...
def collect_matched():
    ...
    return matched[matched["sex"]==2]  # men-only across all strategies


def hybrid_strategy(matched, parlay_min, single_min, edge_min=0.05):
    """For each card, decide: parlay top-2 / single best / nothing.
    Returns list of bets with combined_odds, p, won, type ('parlay' or 'single')."""
    ...
    for date, grp in pos.groupby("DATE"):
        ...
        if len(ranked) >= 2 and ranked.iloc[0]["edge"]>=parlay_min and ranked.iloc[1]["edge"]>=parlay_min:
            ...
            for combo in combinations(top2.itertuples(index=False), 2):
                ...
                bets.append(dict(type="parlay", odds=co, won=won, fold=str(combo[0].fold),
                                 date=str(combo[0].DATE)))
        elif ranked.iloc[0]["edge"] >= single_min:
            r = ranked.iloc[0]
            bets.append(dict(type="single", odds=float(r["dec_odds_pick"]),
                             won=int(r["won_pick"]), fold=str(r["fold"]), date=str(r["DATE"])))
    return bets


def make_pure_parlay(matched, edge_threshold=0.05):
...
def make_pure_singles(matched, edge_threshold=0.05):
    pos = matched[(matched["ev"]>0) & (matched["edge"]>=edge_threshold)].copy()
    return [dict(type="single", odds=float(r.dec_odds_pick), won=int(r.won_pick),
...
def evaluate(bets):
    ...
    n_pos_folds = sum(1 for v in fold_rois.values() if v is not None and v > 0)
    n_total_folds = sum(1 for v in fold_rois.values() if v is not None)
    n_parlay = sum(1 for b in bets if b["type"] == "parlay")
    n_single = len(bets) - n_parlay
    return dict(n=len(bets), n_parlay=n_parlay, n_single=n_single,
                pooled=float(pnl.mean()*100),
                hit=float(np.mean([b["won"] for b in bets])*100),
                ci_lo=float(np.percentile(boots,2.5)),
                ci_hi=float(np.percentile(boots,97.5)),
                f8=fold_rois.get("fold_8"),
                fold_rois={k: round(v,1) if v is not None else None for k,v in fold_rois.items()},
                pos_folds=f"{n_pos_folds}/{n_total_folds}")


def main():
    cache = Path("results/hybrid_matched_lambda150_men.parquet")
    if cache.exists():
    ...
    strategies = [
        # Baselines
        ("STRAIGHT edge≥5pp",                 make_pure_singles(matched, 0.05)),
        ("STRAIGHT edge≥10pp",                make_pure_singles(matched, 0.10)),
        ("PARLAY-2 edge≥5pp top-2 (current)", make_pure_parlay(matched, 0.05)),
        ("PARLAY-2 edge≥10pp top-2",          make_pure_parlay(matched, 0.10)),
        # Hybrids
        ("H1: parlay≥10pp / single≥10pp (user spec)",
            hybrid_strategy(matched, parlay_min=0.10, single_min=0.10)),
        ("H2: parlay≥10pp / single≥5pp",
            hybrid_strategy(matched, parlay_min=0.10, single_min=0.05)),
        ("H3: parlay≥7pp / single≥10pp",
            hybrid_strategy(matched, parlay_min=0.07, single_min=0.10)),
        ("H4: parlay≥5pp / single≥10pp",
            hybrid_strategy(matched, parlay_min=0.05, single_min=0.10)),
        ("H5: parlay≥7pp / single≥7pp",
            hybrid_strategy(matched, parlay_min=0.07, single_min=0.07)),
        ("H6: parlay≥10pp / single≥7pp",
            hybrid_strategy(matched, parlay_min=0.10, single_min=0.07)),
    ]
    ...
    print()
    print("KEY:")
    print("  P|S    = parlays | singles in this strategy")
    print("  CI     = bootstrap 95% on pooled ROI per BET (so parlay variance = single variance unit)")
    print("  F8     = ROI in the most recent quarter (Jan-Apr 2026)")
    print("  pos_f  = folds with positive ROI / folds with any bets")

    Path("results/hybrid_strategy_backtest.json").write_text(json.dumps(rows, indent=2, default=str))
    print(f"\n✓ Saved results/hybrid_strategy_backtest.json")
