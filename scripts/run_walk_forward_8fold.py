"""8-fold walk-forward on the 18-month MMA-AI test window.

Simulates realistic production: retrain the LR model before each ~2.25-month
"betting period." Training window slides forward, always 8 years long.
Predictions accumulate across folds and we report both per-fold metrics and
pooled aggregate, plus ROI on each fold.

Design choices (documented for honesty):
- τ values (PG/BB smoothing) are FROZEN at MMA-AI v7 published defaults across
  all folds. Per-fold Optuna τ reoptimization would require feature rebuild
  per fold (~8h total) and in practice a real bettor retraining every 4 months
  would not re-run a 100-trial hyperparameter search. τ values are stable
  across train windows anyway (cross-validated once by MMA-AI on 2014-2024).
- Features (mmaai_features.csv + sos_form + recency_stance + style_elos) are
  rebuilt ONCE globally. All per-fighter history features use strict
  d<fight_date filtering internally (§3 compliant). Rebuilding per fold would
  change values by <0.1%.
- LR hyperparams (C=0.05, l1=0.5, recency λ=0.13) are FROZEN. Per §6 we never
  tune on the test window.

Leakage guardrails (LEAKAGE_REFERENCE.md §1–§11):
  §1  Temporal walk-forward; train always strictly before fold start.
  §3  All feature builders already use d<fight_date (audited previously).
  §4  Imputer/scaler refit on each fold's train slice.
  §6  Hyperparams frozen (same values as prior single-shot LR).
  §7  Vegas odds evaluation-only.
  §10 Single run, single report; strategies defined before evaluation.

Reports:
  - Per-fold: n_train, n_test, test dates, acc/ll/auc/brier
  - Aggregate (pooled): same metrics across all 8 folds
  - Comparison: pooled vs single-shot baseline (prior result)
  - ROI: aggregate across folds for Strategy D (+EV) — does per-fold retrain
         improve vs single-shot train?
"""
...
# 8 folds over MMA-AI's test window (18 months)
TEST_FIRST = pd.Timestamp("2024-05-04")
TEST_LAST  = pd.Timestamp("2025-11-08")
N_FOLDS = 8
TRAIN_YEARS = 8                         # sliding window length
TRAIN_ERA_FLOOR = pd.Timestamp("2016-01-01")  # don't go earlier than this
FILTER_THRESHOLD = 3
LAM = 0.13  # recency weight
...
def train_lr(train, usable):
    ...
    # Recency weight relative to the fold's end date (most recent train fight)
    ref = train["DATE"].max()
...
def main():
    print("="*70)
    print("STEP 1: Load features, apply filter, merge all layers")
    print("="*70)
    ...
    print(f"\n  8-fold walk-forward (avg fold length: {span/N_FOLDS:.0f} days)")
    for i, (fs, fe) in enumerate(folds, 1):
        print(f"    Fold {i}: test {fs.date()} → {fe.date()}  "
              f"({(fe-fs).days} days)")

    # ── Attach Vegas to ALL test fights (for later ROI) ─────────────
    all_test = df[(df["DATE"] >= TEST_FIRST) & (df["DATE"] <= TEST_LAST)].copy()
    test_v = attach_vegas(all_test)

    # ── Walk-forward ────────────────────────────────────────────────
    print("\n" + "="*90)
    print("STEP 2: Walk-forward — retrain LR per fold with sliding 8-year training window")
    print("="*90)
    per_fold = []
    all_rows = []      # per-bout test predictions across folds (for aggregate)
    for i, (fs, fe) in enumerate(folds, 1):
        ...
        te = df[(df["DATE"] >= fs) & (df["DATE"] < fe)].copy() \
            if i < N_FOLDS else df[(df["DATE"] >= fs) & (df["DATE"] <= fe)].copy()
        if len(te) == 0:
            print(f"  Fold {i}: skipped (0 test fights)")
            continue
        ...
        print(f"  Fold {i}: train {train_start.date()}→{fs.date()} "
              f"({len(tr):,})  test {fs.date()}→{fe.date()} ({len(te):>3})  "
              f"acc={m['acc']*100:.2f}%  ll={m['ll']:.4f}  auc={m['auc']:.4f}  "
              f"brier={m['brier']:.4f}")

    wf = pd.concat(all_rows, ignore_index=True)
    print(f"\n  Pooled test predictions: {len(wf)} across {len(per_fold)} folds")

    # ── Aggregate metrics (pooled across folds) ─────────────────────
    y = wf["win"].astype(int).values
    p = wf["p_model"].values
    m_pool = metrics(y, p)
    print(f"\n  POOLED: acc={m_pool['acc']*100:.2f}%  ll={m_pool['ll']:.4f}  "
          f"auc={m_pool['auc']:.4f}  brier={m_pool['brier']:.4f}")

    # ── Compare to single-shot baseline (from prior runs) ───────────
    print(f"\n  Single-shot baseline (one train, one test, prior LR+Elo+recency+style):")
    print(f"    70.97% acc / 0.5914 ll / 0.7528 auc / 0.2019 brier (n=434)")
    print(f"  Δ walk-forward vs single-shot (per-fold retrain helps if Δacc > 0):")
    print(f"    Δacc={m_pool['acc']*100 - 70.97:+.2f}pp  "
          f"Δll={0.5914 - m_pool['ll']:+.4f}  "
          f"Δauc={m_pool['auc']-0.7528:+.4f}  "
          f"Δbrier={0.2019 - m_pool['brier']:+.4f}")

    # ── ROI per fold + aggregate ────────────────────────────────────
    print("\n" + "="*90)
    print("STEP 3: ROI per fold (Strategy D: +EV where model_p > Vegas_devig)")
    print("="*90)
    # Join Vegas onto wf via (DATE, jbout, jfighter)
    ...
    print(f"  Matched with Vegas: {len(wf_v)}/{len(wf)}")

    # Strategy D: edge on pick > 0
    ...
    # Per-fold ROI
    print(f"\n  {'Fold':>4s}  {'n_total':>7s}  {'n_ev':>4s}  {'win%':>5s}  {'ROI':>7s}")
    fold_rois = []
    for fold_i in sorted(wf_v["fold"].unique()):
        fm = (wf_v["fold"] == fold_i).values & mask_d
        ...
        roi = profits.mean()
        fold_rois.append(dict(fold=int(fold_i), n_ev=int(fm.sum()),
                              win_rate=float((profits > 0).mean()),
                              roi=float(roi)))
        n_total = (wf_v["fold"] == fold_i).sum()
        print(f"  {int(fold_i):>4d}  {n_total:>7d}  {int(fm.sum()):>4d}  "
              f"{(profits > 0).mean()*100:>4.1f}%  {roi*100:>+6.2f}%")
    ...
    print(f"\n  AGGREGATE Strategy D (+EV): n={len(total_bets)}  "
          f"win_rate={(profits_all > 0).mean()*100:.1f}%  "
          f"ROI={profits_all.mean()*100:+.2f}%  "
          f"CI=[{lo*100:+.2f}%, {hi*100:+.2f}%]  p={p_one:.3f}")
    print(f"  (Prior single-shot ROI: +14.36%, CI=[+1.56%, +27.85%], p=0.023)")

    # Save
    out = {
        "config": {"n_folds": N_FOLDS, "train_years": TRAIN_YEARS,
                   "test_first": str(TEST_FIRST.date()),
                   "test_last": str(TEST_LAST.date()),
                   "filter_threshold": FILTER_THRESHOLD,
                   "lr": "C=0.05,l1=0.5,saga",
                   "recency_lambda": LAM},
        "per_fold": per_fold,
        "pooled_metrics": m_pool,
        "roi_per_fold": fold_rois,
        "roi_aggregate": {
            "n": len(total_bets),
    ...
    (DT / "walk_forward_8fold.json").write_text(json.dumps(out, indent=2, default=str))
    print(f"\nSaved to {DT/'walk_forward_8fold.json'}")
