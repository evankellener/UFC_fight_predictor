   ...
   '''
        ...
        for period_idx, (test_start, test_end) in enumerate(test_periods):
            ...
            try:
                ...
                if True:  # Always use logistic regression
                    ...
                    # Use EXACT same parameters as tune_logistic_regression method
                    # Best parameters: C=0.1, class_weight='balanced', penalty='l2', solver='liblinear'
                    # Note: Data is already imputed in _prepare_data, so we don't need imputation here
                    pipeline = Pipeline([
                        ('scaler', RobustScaler()),
                    ...
                    # Fit the pipeline directly without grid search to get exact same results
                    pipeline.fit(X_train, y_train)
                    probs = pipeline.predict_proba(X_test)[:, 1]
                    best_model = pipeline  # Assign the fitted pipeline as best_model
    ...
    def backward_rolling_backtest_roi(self, 
        ...
        for i in range(num_periods):
            # Convert test_period from years to months for relativedelta
            test_period_months = int(test_period * 12)
            test_start = current_end - relativedelta(months=test_period_months)
        ...
        for period_idx, (test_start, test_end) in enumerate(test_periods):
            ...
            try:
                ...
                # For the last period (most recent) with constant_window=False, use the exact same data as tune_logistic_regression
                # Only do this if the test period matches exactly with tune_logistic_regression's test period
                if (period_idx == (num_periods - 1) and not constant_window and 
                    test_start == (most_recent_date - relativedelta(years=1)) and
                    test_end == most_recent_date):
                    # Use the exact same preprocessed data as tune_logistic_regression
                    X_train = self.X_train
                    y_train = self.y_train
                    X_test = self.X_test
                    y_test = self.y_test
                    print(f"Using preprocessed data from tune_logistic_regression")
                    
                    # Skip all preprocessing steps and go directly to model training
                    print(f"Final training set: {len(X_train)} fights")
                    print(f"Final test set: {len(X_test)} fights")
                    
                    # Train model with exact same parameters as tune_logistic_regression
                    pipeline = Pipeline([
                        ('scaler', RobustScaler()),
                        ('clf', LogisticRegression(
                            C=0.1, 
                    ...
                    # Create odds table for ROI calculation
                    odds_table = make_consistent_odds_table(test_data, probs)
                    ...
                    results.append(period_result)
                    continue  # Skip the rest of the preprocessing
                else:
                    # Use EXACT same feature selection and preprocessing as tune_logistic_regression
                    valid_cols = [c for c in self.importance_columns if c in train_data.columns]
