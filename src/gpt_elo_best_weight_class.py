import pandas as pd
# Since direct import failed, we'll manually redefine the EnhancedEloWithInferredWeightClass using the updated logic
...
class EnhancedElo:
    ...
    def relative_weight_modifier(self, f_nat, f_opp, fight_class):
        try:
            ...
            if abs(relative_diff) >= 1:
                ...
                return max(0.75, 1 - 0.08 * abs(relative_diff))  # capped modifier
            return 1.0
        except:
            return 1.0
    ...
    def process_fights(self, df):
        ...
        for i, row in df.iterrows():
            ...
            k_fighter = self.k * method_weight * streak_bonus_fighter * weight_mod
            k_opponent = self.k * method_weight * streak_bonus_opponent * weight_mod
