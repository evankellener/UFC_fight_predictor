...
class SimpleUFCPredictorFixed:
    """
    A fixed version of the UFC fight predictor that properly handles data and features.
    """
    ...
    def _load_data(self):
        ...
        # Less aggressive filtering - only require win column and basic features
    ...
    def _prepare_model(self):
        ...
        self.model = LogisticRegression(
            max_iter=10000,
            C=1.0,  # Reduced from 10 to prevent overfitting
    ...
    def predict_fight(self, fighter1_name, fighter2_name):
        ...
        print(f"Found {len(fighter1_data)} fights for {fighter1_name}")
        print(f"Found {len(fighter2_data)} fights for {fighter2_name}")
        ...
        for col in self.feature_columns:
            if col.startswith('opp_'):
                # For opponent columns, we need to get the opponent's data
                # Since this is fighter1 vs fighter2, opponent columns should use fighter2's data
                base_col = col.replace('opp_', '')
                if base_col in fighter2_latest:
                    features.append(fighter2_latest[base_col])
                    feature_debug.append(f"{col}={fighter2_latest[base_col]}")
                else:
                    features.append(0)
                    feature_debug.append(f"{col}=0 (missing)")
            else:
                # Regular columns use fighter1's data
                if col in fighter1_latest:
                    features.append(fighter1_latest[col])
                    feature_debug.append(f"{col}={fighter1_latest[col]}")
                else:
                    features.append(0)
                    feature_debug.append(f"{col}=0 (missing)")
        
        print(f"Feature vector created with {len(features)} features")
        print(f"Sample features: {feature_debug[:5]}")
        ...
        print(f"Raw prediction probability: {prob_win:.4f}")
