...
class TimeCalibratedClassifier(BaseEstimator, ClassifierMixin):
    def __init__(self, base_estimator=None, method='isotonic',
                 calibration_fraction=0.2, min_cal_size=150, random_state=42):
    ...
    def fit(self, X, y):
        n = len(X)
        cal_size = max(int(n * float(self.calibration_fraction)), int(self.min_cal_size))
        if n <= 1:
            cal_size = 0
        else:
            cal_size = min(cal_size, n - 1)
        split = n - cal_size if cal_size > 0 else n
        X_tr, y_tr = X[:split], y[:split]
        X_cal, y_cal = X[split:], y[split:]
        est = clone(self.base_estimator)
        if hasattr(est, "random_state"):
            est.random_state = getattr(est, "random_state", self.random_state)
        est.fit(X_tr, y_tr)
        if cal_size > 0:
            cal = CalibratedClassifierCV(est, cv='prefit', method=self.method)
            cal.fit(X_cal, y_cal)
            self._calibrated = cal
        else:
            self._calibrated = est
        self.classes_ = getattr(self._calibrated, "classes_", None)
        return self
...
class FightOutcomeModel:
    def __init__(self, file_path, scaler_path=None):
        ...
        self.importance_columns = [
            ...
            'weightindex', 'opp_weight_of_fight', 'REACH', 'opp_REACH', 'opp_precomp_tdavg'
        ]
    ...
    def _make_roi_scorer(self, vegas_data_path, meta_source_df, stake=100):
        import os, uuid
        import pandas as pd
        import numpy as np
        def roi_scorer(estimator, X_val, y_val):
            meta_cols = ['DATE', 'EVENT', 'BOUT', 'FIGHTER']
            try:
                meta = meta_source_df.loc[X_val.index, meta_cols].copy()
            except Exception:
                return 0.0
            try:
                probs = estimator.predict_proba(X_val)[:, 1]
            except Exception:
                z = estimator.decision_function(X_val)
                probs = 1.0 / (1.0 + np.exp(-z))
            odds_df = make_consistent_odds_table(meta, probs)[['DATE','EVENT','BOUT','FIGHTER','odds']]
            tmp_dir = os.path.join('data', 'tmp', 'roi_cv_tmp')
            os.makedirs(tmp_dir, exist_ok=True)
            tmp_path = os.path.join(tmp_dir, f'cv_{uuid.uuid4().hex}.csv')
            odds_df.to_csv(tmp_path, index=False)
            picks = self.calculate_roi(odds_table_path=tmp_path, vegas_data_path=vegas_data_path)
            if isinstance(picks, pd.DataFrame) and len(picks):
                total_stake = picks['stake'].sum() if 'stake' in picks.columns else stake * len(picks)
                if total_stake == 0:
                    return 0.0
                roi = float(picks['profit'].sum()) / float(total_stake)
                return roi
            return 0.0
        return roi_scorer

    def tune_logistic_regression_roi(self, feature_list=None, vegas_data_path=None):
        from sklearn.pipeline import Pipeline
        from sklearn.impute import SimpleImputer
        from sklearn.preprocessing import RobustScaler
        from sklearn.linear_model import LogisticRegression
        ...
        if feature_list is None:
            feature_list = list(self.importance_columns)
        X_tr = self.train_df[feature_list].copy()
        y_tr = self.train_df['win'].copy()
        scorer = self._make_roi_scorer(vegas_data_path=vegas_data_path, meta_source_df=self.train_df)
        pipe = Pipeline([
            ('imputer', SimpleImputer(strategy='median')),
            ('scaler', RobustScaler()),
            ('clf', TimeCalibratedClassifier(
        ...
        param_grid = {
            ...
            'clf__base_estimator__solver': ['liblinear', 'saga'],
            'clf__base_estimator__class_weight': [None, 'balanced'],
            'clf__method': ['isotonic', 'sigmoid'],
            'clf__calibration_fraction': [0.15, 0.2, 0.25],
        }
        tscv = TimeSeriesSplit(n_splits=5)
        grid = GridSearchCV(
            estimator=pipe,
        ...
        grid.fit(X_tr, y_tr)
        best_model = grid.best_estimator_
        best_roi = float(grid.best_score_)
        self.model = best_model
        self.probs = best_model.predict_proba(self.test_df[feature_list])[:, 1]
        self.last_features_used = feature_list
        print(f"Best ROI (CV mean): {best_roi:.4f}")
        print(f"Best params: {grid.best_params_}")
        return best_model, best_roi
    ...
    def roi_optimized_features(self, base_features=None, vegas_data_path=None, stake=100, top_n=5):
        """
        Find the best features to add based on ROI performance using the CURRENT trained model.
        This method uses the existing model and data splits to ensure accuracy.
        
        Args:
            base_features: List of base features to start with (defaults to importance_columns)
            vegas_data_path: Path to Vegas odds data for ROI calculation
            stake: Betting stake per fight (default $100)
            top_n: Number of top features to return (default 5)
            
        Returns:
            tuple: (best_features_list, results_df) - List of best features and full results
        """
